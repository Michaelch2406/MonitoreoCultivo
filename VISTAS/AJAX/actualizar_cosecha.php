...
try {
    ...
    if (!in_array($usuario_actual['rol'], ['administrador', 'agricultor', 'supervisor'])) {
        echo json_encode([
            'success' => false,
            'message' => 'No tienes permisos para actualizar cosechas'
        ]);
        exit();
    }
    
    // Validar ID de cosecha
    if (!isset($_POST['cosecha_id']) || !is_numeric($_POST['cosecha_id'])) {
        echo json_encode([
            'success' => false,
    ...
    $cosecha_id = intval($_POST['cosecha_id']);
    
    // Validar datos requeridos
    $campos_requeridos = ['fecha_cosecha', 'cantidad_cosechada', 'unidad', 'calidad'];
    ...
    $resultado = $cosecha_modelo->actualizarCosecha($cosecha_id, $datos_cosecha, $usuario_actual['id'], $usuario_actual['rol']);
    ...
    error_log("Error al actualizar cosecha: " . $e->getMessage());
    echo json_encode([
        'success' => false,
